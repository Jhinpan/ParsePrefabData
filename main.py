...
def extract_prefab_data(file_path):
    # Regular expressions to extract necessary details
    ...
    # Exclude patterns
    exclude_patterns = ["ObjectHighlighter", "Environment_", "BFW180201_", "solid", "glass_", "Wall", "xWalls", "zWalls", "Rooms", "RoomTypes", "Bedroom1", "Bedroom2", "livingroom",
                        "Window", "Environment", "Floor", "CreatedObjects", "CreatedBoxes", "BoxGroup", "visuals", "link_", "midpoint_",
    ...
    gameobject_names = gameobject_name_pattern.findall(content)
    local_positions = local_position_pattern.findall(content)
    local_rotations = local_rotation_pattern.findall(content)
    local_scales = local_scale_pattern.findall(content)

    data = list(zip(gameobject_names, local_positions, local_rotations, local_scales))

    # Filter out the unwanted data
    filtered_data = [item for item in data if not any(pattern in item[0] for pattern in exclude_patterns)]

    return filtered_data


def parse_directory_for_prefabs(root_directory):
    all_prefab_data = {}
                ...
                if os.path.exists(c_folder):
                    ...
                    for file in os.listdir(c_folder):
                        if file.startswith("Environment_") and file.endswith(".prefab"):
                            ...
                            all_prefab_data[f'P{p_index}'][f'C{c_index}'][f'E{e_number}'] = extract_prefab_data(
                                full_path)
...
all_data = parse_directory_for_prefabs(root_directory)
